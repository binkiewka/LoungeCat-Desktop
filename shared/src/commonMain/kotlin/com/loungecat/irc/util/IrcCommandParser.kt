...
object IrcCommandParser {
    ...
    fun parse(message: String): IrcCommand {
        ...
        return when (command) {
            ...
            "query", "q" -> {
                ...
                if (queryParts.isEmpty() || queryParts[0].isBlank()) return IrcCommand.Unknown(command, args)
            ...
            "kick", "k" -> {
                ...
                if (kickParts.isEmpty() || kickParts[0].isBlank()) return IrcCommand.Unknown(command, args)
    ...
    fun getHelpText(): String = """
        ...
        |MESSAGING: /msg /query /me /notice
        |USER: /nick /whois /who /away /back /ignore /unignore
        |MODERATION: /kick /ban /unban /voice /devoice /op /deop /quiet /unquiet /mode
        |SERVER: /quit /raw /ping /time /version /motd /info
        |CLIENT: /clear /help
        |
